...
Node* insertionSort(struct Node* head)
{
    ...
    // Initialize pointers to track the previous and current nodes
    Node * prev = head, * curr = head->next;

    // Iterate through the list starting from the second node
    while( curr != NULL) {

        // If the current node's data is greater than or equal to the previous node's data
        if( curr->data >= prev->data) {
            ...
            // Create a temporary pointer pointing to the dummy node
            Node * temp = dummy;
            ...
            // Adjust the pointers to insert the current node into its correct position
            prev->next = curr->next;
            curr->next = temp->next;
            temp->next = curr;
            curr = prev->next;
    ...
    // Return the sorted list starting from the next of the dummy node
    return dummy->next;
}